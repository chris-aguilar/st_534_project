...
We will use the best-fitting model, $ARIMA(3,0,0)\times(0,1,1)_4$ with a constant, to generate forecasts for the next 3 years forecast.

```{r forecasting, message=FALSE, warning=FALSE, results='hide'}
library(fpp3)

fit <- aviation_data |>
  model(arima300_011_c = ARIMA(log(n) ~ 1 + pdq(3, 0, 0) + PDQ(0, 1, 1, period=4)
    )
  )
